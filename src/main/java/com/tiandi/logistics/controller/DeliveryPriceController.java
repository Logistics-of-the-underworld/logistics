...
public class DeliveryPriceController {
    ...
    public ResultMap getAllDeliveryPrice(@RequestHeader String tooken,@PathVariable(value = "nameCompany",required = false) String name_company){
        ...
        if ("root".equals(permission) && "admin".equals(role)){
            ...
            while (iterator.hasNext()){
                ...
                /*根据配送点编码获取配送点价格编码*/
                DeliveryPriceRange id_distribution = deliveryPriceRangeService.getBaseMapper().selectOne(new QueryWrapper<DeliveryPriceRange>().eq("id_distribution", idDistribution));
                String idDeliveryPrice = id_distribution.getIdDeliveryPrice();
                /*根据配送点编码获取配点价格信息*/
    ...
    public ResultMap getDeliveryPriceByID(@RequestHeader String token,@PathVariable(name = "idDistribution") String id_distribution){
        ...
        if (permission.equals("admin") && role.equals("distribution") && id_distribution != null){
            ...
            Integer idRange = id_distribution1.getIdRange();
            /*根据配送点编码获取配送价格编码*/
            DeliveryPriceRange id_distribution2 = deliveryPriceRangeService.getBaseMapper().selectOne(new QueryWrapper<DeliveryPriceRange>().eq("id_distribution", id_distribution));
            ...
            /*根据配送价格编码获取配送价格*/
            DeliveryPrice id_delivery_price = deliveryPriceService.getBaseMapper().selectOne(new QueryWrapper<DeliveryPrice>().eq("id_delivery_price", idDeliveryPrice));
            DeliveryPrice id_delivery_price1 = deliveryPriceService.getBaseMapper().selectOne(new QueryWrapper<DeliveryPrice>().eq("id_delivery_price", idDeliveryPrice1));
            /*把配送点的配送价格和配送范围的配送价格添加到集合*/
            List<DeliveryPrice> list = new ArrayList<>();
            list.add(id_delivery_price);
            list.add(id_delivery_price1);
